...
const ProductCard = ({ product, addToCartHandler }) => {
    ...
    return (
        <div className="bg-white/5 rounded-2xl overflow-hidden group hover:shadow-[0_20px_50px_rgba(0,0,0,0.5)] transition-all duration-500 border border-white/5 hover:border-primary/30 relative flex flex-col h-full active:scale-[0.98] backdrop-blur-md">
            ...
            <div className="h-52 overflow-hidden relative bg-white/5">
                {!imageLoaded && (
                ...
                {/* Weight badge - More compact */}
                <div className="absolute bottom-3 left-3">
                    <span className="bg-primary backdrop-blur-md text-white text-[9px] font-black px-2.5 py-0.5 rounded-full uppercase tracking-widest shadow-xl">
                        250g • Handcrafted
                    </span>
                </div>
            </div>
            ...
            <div className="p-4 flex flex-col flex-grow">
                <div className="text-[9px] text-primary mb-1.5 uppercase tracking-[0.2em] font-black">{product.category || 'Premium Pickle'}</div>

                <Link to={`/product/${product._id}`}>
                    <h3 className="text-base font-serif font-black text-light mb-2 hover:text-primary transition-colors leading-tight line-clamp-2">
                        {product.name}
                    </h3>
                </Link>

                <div className="flex items-center space-x-2.5 mb-4">
                    <span className="text-xl font-black text-light">₹{product.price}</span>
                    <span className="text-xs text-gray-500 line-through font-medium">₹{Math.round(product.price * 1.2)}</span>
                    <span className="text-[9px] text-secondary font-black uppercase tracking-wider bg-secondary/10 px-2 py-0.5 rounded-full border border-secondary/20">Save 20%</span>
                </div>
                ...
                <div className="mt-auto">
                    <button
                        ...
                        className={`w-full ${product.countInStock === 0 ? 'bg-gray-800 text-gray-500 cursor-not-allowed' : 'bg-white/10 text-white hover:bg-primary'} py-2.5 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] transition-all duration-300 border border-white/10 hover:border-transparent active:scale-95 flex items-center justify-center gap-2 group/btn`}
                    >
